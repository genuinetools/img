...
Docker image `r.j3ss.co/img` is configured to be executed as an unprivileged user with UID 1000 and it does not need `--privileged` since `img` v0.5.8.
...
Since `img` v0.5.8, you don't need to specify any `securityContext` for running `img` as a Kubernetes container.
